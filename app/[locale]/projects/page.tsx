...
export async function generateMetadata({
  ...
  const metadata = dictionary.projectsPage.metadata;
    ...
    <main className="bg-[#EAF1F6] text-slate-900">
      <section className="relative overflow-hidden bg-[#0b3557] px-6 py-24 text-white">
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            ...
            >
              {projectsPage.hero.primaryLabel}
            ...
            >
              {projectsPage.hero.secondaryLabel}
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="rounded-3xl border border-sky-200 bg-sky-50/80 px-8 py-8 shadow-sm md:px-10">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-sky-700">
              {projectsPage.portfolioNotice.eyebrow}
            </p>

            <p className="mt-4 max-w-4xl text-lg leading-8 text-slate-700">
              {projectsPage.portfolioNotice.description}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="relative mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-sky-700">
              {projectsPage.areas.eyebrow}
            </p>

            <h2 className="mt-4 text-4xl font-bold leading-tight text-slate-950 md:text-5xl">
              {projectsPage.areas.title}
            </h2>

            <p className="mt-6 text-lg leading-8 text-slate-600">
              {projectsPage.areas.description}
            </p>
          </div>

          <div className="mt-12 grid gap-8 lg:grid-cols-2">
            {projectsPage.areas.items.map((project) => (
              <article
                key={project.number}
      ...
      <section className="bg-white px-6 py-20 md:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-sky-700">
              {projectsPage.approach.eyebrow}
            </p>

            <h2 className="mt-4 text-4xl font-bold leading-tight text-slate-950 md:text-5xl">
              {projectsPage.approach.title}
            </h2>

            <p className="mt-6 text-lg leading-8 text-slate-600">
              {projectsPage.approach.description}
            </p>
          </div>

          <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {projectsPage.approach.stages.map((stage) => (
              <article
                key={stage.number}
      ...
      <section className="px-6 py-20 md:py-24">
        <div className="mx-auto max-w-7xl overflow-hidden rounded-3xl bg-slate-950 px-8 py-14 text-white shadow-[0_28px_80px_rgba(15,23,42,0.22)] md:px-14">
          <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-sky-300">
                {projectsPage.callToAction.eyebrow}
              </p>

              <h2 className="mt-4 max-w-3xl text-3xl font-bold leading-tight md:text-5xl">
                {projectsPage.callToAction.title}
              </h2>

              <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-300">
                {projectsPage.callToAction.description}
              </p>
            </div>
            ...
            >
              {projectsPage.callToAction.buttonLabel}
              <span aria-hidden="true">→</span>
            </Link>
          </div>
        </div>
